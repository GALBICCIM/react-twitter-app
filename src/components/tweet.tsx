	...
	return (
		<Style.Wrapper>
			<Style.Columns>
				...
				{isEditing ? <Style.EditInput type="text" value={editTweet}></Style.EditInput> : <Style.Payload>{tweet}</Style.Payload>}
				{user?.uid === userId ? (
					<Style.Columns>
						<Style.DeleteButton onClick={onDelete}>Delete</Style.DeleteButton>
						<Style.EditButton onClick={onEdit}>{isEditing ? "Confirm" : "Edit"}</Style.EditButton>
					</Style.Columns>
				) : null}
			</Style.Columns>
			{photo ? (
